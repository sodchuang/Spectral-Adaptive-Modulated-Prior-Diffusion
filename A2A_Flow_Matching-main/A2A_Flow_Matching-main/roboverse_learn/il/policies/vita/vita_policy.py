...
Implementation of paper "VITA: Vision-to-action flow matching policy." 
arXiv preprint arXiv:2507.13231 (2025).

Architecture:
    Visual Obs [img_{t-n+1}, ..., img_t] --encode--> obs_latents (x_0)
    
    Flow Matching: x_0 --flow--> x_1 (action_latents)
    
    x_1 --decode--> Actions [a_0, a_1, ..., a_{horizon}]
"""
...
class VITAImagePolicy(BaseImagePolicy):
    ...
    Flow Matching from visual observation latents to action latents.
    
    - Flow START: Visual observation latents (obs_latents)
    - Flow TARGET: Action latents (action_latents)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Flow network (no condition for original VITA)
        ...
        # Action encoder/decoder for full horizon
    ...
    def compute_loss(self, batch):
        ...
        action_latents = self.action_encoder(nactions)
            ...
            # Flow matching: obs_latents -> action_latents
            flow_loss, metrics = self.flow_matcher.compute_loss(
                self.flow_net,
            target=action_latents,
            start=obs_latents,  # Visual latents as flow source
            )
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Flow sampling: obs_latents -> action_latents
            action_latents_pred = self.flow_matcher.sample(
                self.flow_net,
        ...
        # Extract action chunk starting from n_obs_steps-1
